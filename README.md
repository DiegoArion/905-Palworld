...
Bot de Discord que reporta en un canal específico lo que pasa en un servidor de Palworld hosteado en **Dathost**: entradas/salidas de jugadores, estado del servidor, tiempo jugado (con leaderboard automático), y — si el log lo permite, ver sección de limitaciones — chat, muertes y capturas de Pals.
...
- `DATHOST_SERVER_ID`: corré `npm run list-servers` (ver paso 5) una vez tengas el `.env` con email/password cargados, y copiá el `id` del server de Palworld que te interese del JSON que imprime.
- RCON del server de Palworld (panel de Dathost → tu servidor):
  - `PALWORLD_RCON_HOST` y `PALWORLD_RCON_PORT`: pasá el mouse sobre la IP del server en el panel izquierdo.
  - `PALWORLD_RCON_PASSWORD`: pestaña **Settings** del server.
  - Confirmá que RCON esté habilitado para ese servidor (mismo panel).
...
- Joins/leaves en tiempo real (vía RCON)
...
src/
  ...
  dathostClient.js      # API REST de Dathost
  rconClient.js         # RCON (ShowPlayers, Info)
